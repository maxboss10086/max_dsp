...
// Descriptions:    DMA将spi的数组送入FIFO或从FIFO中读取数据送入数组
// Author:          Max
// Creation Date:   2020年5月24日
//----------------------------------------------------------------------------------------//
//note:0DMA代替CPU将数据和FIFO进行交互,无需CPU执行数据的搬运工作
//     1spi的发送FIFO信号不再触发SPI中断，而是触发DMA
//     2dma，就是sdram控制器，与FIFO的联合工作参考正点原子SDRAM控制器
//     3采用连续模式，则接收FIFO达到要求就进行DMA传输，接收数组中的旧数据会被覆盖
//     4不采用连续模式，则接收FIFO达到要求就进行DMA传输，接收数组中的被填满，则DMA停止传输
//     5采用连续模式，则发送数组中的数据会一直发送，不会在意是不是有更新
//----------------------------------------------------------------------------------------//
//***************************************include******************************************//
...
void init_dma_5(){
    ...
    // buffer register.
    //小于设定FIFO深度时，DMA通道5将Sdata送入发送FIFO
    DMA_configAddresses(DMA_CH5_BASE, (uint16_t *)(SPIB_BASE + SPI_O_TXBUF),spib_sData);
    DMA_configBurst(DMA_CH5_BASE, BURST, 1, 0);//突发传输
