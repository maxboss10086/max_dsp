...
void clb1_in7_init_set(){
    ...
    CLB_configGlobalInputMux(CLB1_BASE, CLB_IN7, CLB_GLOBAL_IN_MUX_CLB_AUXSIG1);//外来信号接入clb_xbar
}
...
//    XBAR_enableCLBMux(XBAR_AUXSIG1, XBAR_MUX05);
    ...
    CLB_configGPInputMux(CLB2_BASE, CLB_IN0, CLB_GP_IN_MUX_GP_REG);//endat时钟的默认电平
...
void clb2_in1_init_set(){
    ...
    CLB_configGlobalInputMux(CLB2_BASE, CLB_IN1, CLB_GLOBAL_IN_MUX_EPWM4_CTRDIR);
}
void clb2_in2_init_set(){

    //EPWMA信号输入in2
    CLB_configGPInputMux(CLB2_BASE, CLB_IN2, CLB_GP_IN_MUX_EXTERNAL);//输入epwm经过修改得到endat_clk
...
void clb2_in3_init_set(){
    ...
    //EPWMB信号输入in3
    CLB_configGPInputMux(CLB2_BASE, CLB_IN3, CLB_GP_IN_MUX_EXTERNAL);//输入epwm经过修改得到spi_clk
    CLB_configLocalInputMux(CLB2_BASE, CLB_IN3, CLB_LOCAL_IN_MUX_GLOBAL_IN);
    CLB_configGlobalInputMux(CLB2_BASE, CLB_IN3, CLB_GLOBAL_IN_MUX_EPWM1B);
}
//还没用到
...
void clb2_ou5_init_set(){
    ...
    //将clb的out5输出接入output xabr
    XBAR_setOutputMuxConfig(OUTPUTXBAR_BASE, XBAR_OUTPUT7, XBAR_OUT_MUX07_CLB2_OUT5);
    XBAR_enableOutputMux(OUTPUTXBAR_BASE, XBAR_OUTPUT7, XBAR_MUX07);
    //将GPIO7复用为output xbar，信号就从GPIO7输出
    GPIO_setPadConfig(16, GPIO_PIN_TYPE_STD);//推完输出
    GPIO_setPinConfig(GPIO_16_OUTPUTXBAR7);//引脚复用为outxbar
}
//设置CLB的输出,clb修改模块的信号，用CLB_setOutputMask函数输出
//clb自己经过逻辑产生的信号，用outxbar输出
...
void clb_init_set(){
    ...
    initTILE2(CLB2_BASE);
    clb2_input_init_set();
    clb2_output_init_set();//out4接入GPIO9，用以控制数据收发
}
